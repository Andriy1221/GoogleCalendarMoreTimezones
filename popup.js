...
document.addEventListener('DOMContentLoaded', () => { 
    ...
    for (let i=0; i < timezones.length; i++) {
        ...
        option.innerHTML = timezones[i].label
        timezoneSelect.appendChild(option)
    }

    //pass selected timezone to content-script.js to update the DOM
    const updateBtn = document.getElementById('updateBtn')
    updateBtn.addEventListener('click', () => {
        console.log(timezoneSelect.value)
        // chrome.runtime.sendMessage({timezone: timezoneSelect.value}, (res) => {
        //     console.log(res)
        // })
        chrome.tabs.query({active: true, currentWindow: true},function(tabs) {
            chrome.tabs.sendMessage(tabs[0].id, {timezone: timezoneSelect.value});
          }); 
    })
